...
//#include "Engine.h"
//
//Main::Main(UINT width, UINT height, std::wstring name) :
...
//	Main::Device::SetDevice(m_device);
//	Main::InitializeDefault::Initialize(m_device.Get(), &m_bindlessHeapDescriptor.GetInstance());
//	Main::Component::Initialize();
//
//
...
//
//	m_gbufferPass = std::unique_ptr<Main::GBufferPass>(new Main::GBufferPass(m_device, m_width, m_height));
//	m_lightingPass = std::unique_ptr<Main::LightingPass>(new Main::LightingPass(m_device, m_width, m_height));
//	m_gizmosPass = std::unique_ptr<Main::GizmosPass>(new Main::GizmosPass(m_device, m_width, m_height));
//	m_compositionPass = std::unique_ptr<Main::CompositionPass>(new Main::CompositionPass(m_device, m_width, m_height));
//	m_uiPass = std::unique_ptr<Main::UIPass>(new Main::UIPass(Win32Application::GetHwnd(), m_device, m_directCommandQueue, FrameCount, m_width, m_height));
//	m_camera = std::unique_ptr<Main::Camera>(new Main::Camera(XMConvertToRadians(60.0f), m_width, m_height, 0.1f, 100000.0f));
//
...
//		entityManager.addComponent(entity, Main::Component::ComponentTestA{ .x = 10.0 }, Engine::Component::ComponentTestB{ .x = 20.0 }, Engine::Component::ComponentTestC{ .x = 20.0 });
//		entityManager.addComponent(entity, Main::Component::ComponentTestA {.x=20.0}, Engine::Component::ComponentTestC{ .x = 30.0 });
//		entityManager.addComponent(entity, Main::Component::ComponentTestB{ .y = 50.0 });
//		entityManager.removeComponent<Main::Component::ComponentTestA>(entity);
//
//		auto entity1 = entityManager.createEntity();
//		entityManager.addComponent(entity1, Main::Component::ComponentTestB{ .y = 120.0 });
//
//
//		auto& o = entityManager.getComponent<Main::Component::ComponentTestB>(entity);
//		auto& o1 = entityManager.getComponent<Main::Component::ComponentTestB>(entity1);
//
//
//
...
//}
//
//void Main::OnMouseUpdate(DX::Mouse::State state) {
//	trackerMouse.Update(state);
//}
//
//void Main::OnKeyboardUpdate(DX::Keyboard::State state) {
...
//}
//
//Main::CommandLists Main::PopulateCommandLists()
//{
//	auto commandLists = Main::CommandLists();
...
//}
//
//void Main::WaitForCommandQueueExecute()
//{
//
//	ThrowIfFailed(m_directCommandQueue->Signal(m_fence.Get(), ++m_fenceValue));
//
